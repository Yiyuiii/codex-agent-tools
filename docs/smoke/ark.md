...
- `ark-coding-plan` → provider `ark-coding-plan` / model `ark-code-latest`；最新批次的 delegate/review 均通过，但同一原子批次随后在 ordinal 6 `ark-agent-plan/delegate` 因 `account_quota_exceeded` 停止，未形成 8/8，因此这两项不能单独晋级，注册表仍为 pending/pending。
...
Gemini 退役后，活动产品面共有四个逻辑 LLM、八项能力，过渡注册表为 6 passed / 2 pending；两个 pending 项都属于 `ark-coding-plan`，安装状态保持 `blocked / not ready`。最新 `four-llm-v1` 批次按首错停为 6 completed / 5 passed、ordinal 7–8 notRun，并以 `blocked / case_failed` 结束；虽然 Coding Plan 两项在该批内通过，但原子晋级要求仍是完整同批 8/8。不能只补跑失败项，也不能把本轮任一通过项与既有证据拼接。

2026-07-27 的 105 秒资格承载演练只构成离线基础设施证据。后续真实批次由单个 `functions.exec` cell 正常承载到协调器终态，证明控制层承载路径有效，但不证明四小时存活。执行边界见[承载手册](../release/four-llm-qualification-execution-runbook.md)，演练事实见[承载演练报告](../release/qualification-carrier-rehearsal.md)。171 files / 15 Markdown/HTML / 3 plugin files 是旧候选的 npm dry-run 历史数字；当前 pack dry-run 为 191 files 且没有持久 `.tgz`。该闭包不改变任何 Ark 路由、证据或资格状态，也不表示已经发布或安装。

最新审计记录：2026-07-28 在 frozen commit `cb9434b4b540e70f5384224e4e98823a3ea2dbae` 上只调用一次标准入口，启动 `four-llm-v1` 批次 `2026-07-28T10-56-09.704Z-649886e3-233e-4da1-ac80-185227342bef`。ordinal 1–5 passed；ordinal 6 `ark-agent-plan/delegate` 的 provider `ark-agent-plan`、模型 `ark-code-latest`、direct route、Agent Plan 凭据隔离、single-attempt 与零 retry/fallback 均正确，结果文件有效，但以 `account_quota_exceeded` failed；ordinal 7–8 notRun。该项 `filesChanged` 包含预期结果文件与来源不明的 `where.cmd`，18 个 Pi bash 生命周期全部为 error，精确写入命令也是 error。现有证据不能把 shell 缺陷写成额度失败原因，也不能证明 `where.cmd` 来自何处。
