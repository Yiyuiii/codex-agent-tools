...
- `ark-coding-plan` → provider `ark-coding-plan` / model `ark-code-latest`；2026-07-25 复跑的 review passed、delegate 因结果文件内容不符而 failed；上一轮四模型批次中 delegate 也在内容验收层 failed，最新四模型批次只持久化到该 delegate 的 `case_running` 后因进程中断结束、没有形成 case evidence，注册表两项仍为 pending。
