...
- 五模型候选复核：运行时代码已收敛到 Direct 两项指纹继续 current 的既有实现；十项历史 evidence 均 valid，Kimi/Ark 八项 stale；尚未运行新的真实模型调用
...
2. 五模型 beta 仍受 Kimi/Ark 八项 stale 阻断；Ark Coding Plan 额度恢复后，按既有单次、首错停协议启动新的 `four-llm-v1` 批次。
