...
3. **原生 observer与启动层（实现已完成）**：`4ccc715`、`0feef26`与`1c82fec`依次完成纯托管严格核心、当前Windows x64 kernel peer/两层祖先 held handles与严格会话材料绑定；`228c6a3`把 `Program` 接到完整双 pipe 会话，使用 held handle 的真实 exit FILETIME、严格EOF、五帧事件、派生marker absence及唯一no-replace receipt writer。MCP客户端同时锁存自然终态并排空`HELLO + 5`，真实socket只以`close`为关闭完成；全局shutdown不能吞掉已锁存终态。固定stdout仅含`READY / OLD_HOST_BOUND / OLD_HOST_EXITED / REQUEST_STARTED`四个动作门，无`PASS`。`5046ab9`固定当前Windows x64/net48 artifact及15项严格build-input manifest；`4a0b3d0`新增只服务当前宿主的固定启动层，核对精确beta tag、公共npm和实际官方plugin cache，使用nonce隔离delegate workspace及正常完成marker，不build/restore/install/publish、不读活动配置，也不设置外部CLI默认/全局限制。真实本机发现并修复`npm.cmd`无法被Node `execFile`启动的问题，现由当前`node.exe`直接运行固定受信`npm-cli.js`。主代理最终复验managed 34/34、真实kernel 7/7、release 206/206、全库69文件1224 passed / 5 skipped、类型、构建、语法、真实registry查询与diff PASS；fresh终审无P1/P2/P3。局部fixture watchdog不进入production，也不构成Kimi、Pi或其它外部CLI的全局限制。只剩current-host freeze、stale-only资格恢复和发布。
...
Task 8 current-host freeze 已完成并通过fresh集成终审：它绑定runtime commit `0af4359c428006408d8c82171c0a33e74096042a`，机器复核8份历史证据有效且8项当前指纹全部stale，三项当前宿主native/observer验证均通过；freeze receipt SHA256为`1a5098682f6b5fb46554f0989e196d7dd420072ef28e5a92188fa31a956a297e`。该事实只批准进入stale-only真实资格阶段，不等于beta或stable发布批准。
