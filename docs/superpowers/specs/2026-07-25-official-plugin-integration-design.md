...
- `doctor` 不读取活动 `~/.codex/config.toml`，不接受 `--config`，也不检查历史 MCP 注册。
- 历史配置文本变换器与公开 install/uninstall/restore/cutover 路径已经删除，不再把测试副本配置当作当前运维能力。
- 项目不得通过“先备份再写入”规避活动配置禁令。

用户已同意采用 Codex 官方插件机制管理插件状态，但该同意不替代对真实写入行为的取证。官方文档明确说明插件开关状态存储在 `~/.codex/config.toml`，所以真实官方安装和升级确定属于可能触碰活动配置的操作。隔离门禁必须先记录官方安装器的精确状态差异；执行前仍需向用户展示必要性、预计精确差异、验证和回滚方案，并取得针对该次官方安装或升级操作的明确许可。
...
1. 根据隔离取证确认官方安装会触碰哪些真实状态，包括活动 `config.toml` 的预计精确差异；先取得针对该次操作的明确许可。
...
- 插件版本以 `package.json` 和 `plugin.json` 为准，两者必须一致；marketplace 条目必须解析到包含该 manifest 的同一插件副本。
- 升级先在新的隔离 `CODEX_HOME` 中重复安装、启动、状态差异和卸载验证。
- 真实升级前比照 §12.2 执行状态差异确认，并针对活动 `config.toml` 的预计精确差异取得该次升级的明确许可。
...
- 单文件 bundle 无外部生产依赖
- staging 产物无开发机绝对路径
- doctor 不读取任何 Codex 配置，CLI 明确拒绝 `--config`，也不检查历史 MCP 注册
...
- Codex 当前手册把 `codex plugin` 和 `codex plugin marketplace` 列为稳定命令，说明已安装插件可以提供 MCP 服务，并明确插件开关状态存储在 `~/.codex/config.toml`。
