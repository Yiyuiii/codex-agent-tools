...
- 现行 `capabilities.json` 的八份历史 evidence 仍可由 immutable verifier 验证，但资格协议与共享 evidence 代码已变化，因此八项旧能力的运行时指纹全部 stale；Direct DeepSeek 两项能力仍缺失。
...
- 两批均取得可复用的 passed case 后，才能把 Direct DeepSeek 两项 gate 改为 passed，并把十项精确能力写入 `capabilities.json`。
