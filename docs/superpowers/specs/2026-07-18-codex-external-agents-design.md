...
状态：历史基线；安装、网络策略、目标模型面与真实切换部分均由 [2026-07-25 官方插件集成设计](2026-07-25-official-plugin-integration-design.md) 取代

日期：2026-07-18

目标仓库：`D:\Codes\codex-agent-tools`

> 本文件记录历史实现。凡与当前官方插件设计冲突的模型、网络、install/uninstall、cutover 或配置写入描述，均为“历史实现，当前已禁用，不得用于活动配置”，不能作为当前运维指引。
...
本节是历史网络设计，当前已由 2026-07-25 官方插件设计覆盖；其中未使用的 11808 分支已经删除，不得据此恢复当前路由。
...
本节是历史实现，当前已禁用，不得用于活动配置。当前公开 CLI 只保留只读 `doctor`，真实集成只允许在逐动作授权后使用 Codex 官方插件命令。
...
本节是历史迁移草案，不是当前执行指引；当前共存、逐动作授权和四层门禁以 2026-07-25 官方插件设计为准。
