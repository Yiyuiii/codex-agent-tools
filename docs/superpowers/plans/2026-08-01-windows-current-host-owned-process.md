...
**资格执行状态（2026-08-03，等待目标恢复）：** 批次 `2026-08-02T14-17-27.683Z-307be99a-f536-4113-8738-51de40b56634` 在 frozen commit `9c40d2440b9ee30defdf92af11156e288785e755` 上从唯一标准入口启动。首项 `ark-coding-plan/delegate` 只调用一次并以 `account_quota_exceeded` 失败；零 retry/fallback、owned process 已排空，其余七项依首错停止合同未运行。终态 immutable verifier 通过，manifest SHA256为`da28beb431619adee599861c9b3cebbeb3d843496243be22ba39140b05d0d157`，证据提交为`29d8673`。维护者现已报告额度恢复；这允许在新的clean candidate上按同一stale-only合同启动独立新批，但不是通过证据，且当前系统 goal 必须先恢复为active。代码修复、Node矩阵、补跑或fallback仍不成立。
