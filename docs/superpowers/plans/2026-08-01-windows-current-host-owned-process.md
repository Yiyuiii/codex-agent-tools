...
**原生observer第四子批状态（2026-08-02，行为已完成）：** `228c6a3 feat: run current-host acceptance observer`将`Program`接入raw stdin、固定脱敏失败输出与完整`HostAcceptanceObserverSession.Run`。首个受保护pipe只在DACL确认后发布`READY`，旧kernel peer与两层祖先handle持有后发布`OLD_HOST_BOUND`；三层真实退出、严格EOF、首pipe完整dispose并成功创建同名受保护第二pipe后才发布`OLD_HOST_EXITED`；新宿主首个请求帧到达后发布`REQUEST_STARTED`，五帧与严格EOF完成、所有pipe/connection/handle释放且marker确认不存在后，receipt才作为最后一步原子写入。MCP客户端自然终态会排空`HELLO + 5`，全局shutdown不能降级该终态，socket close只等真实`close`。fresh综合审阅无P1/P2；两个P3测试项已用held handle FILETIME精确比对和测试事件句柄释放闭合。主代理复验managed 34/34、kernel 7/7、客户端26/26、全库67文件1204 passed / 3平台条件skipped、类型、library build与diff PASS。`e922276`另将stdio cleanup测试绑定到同一当前宿主helper解析及真实`session_shutdown + ownedProcessDrained:true`证据。下一子批只处理observer artifact/build manifest/start script及既有去冗余尾项，不冻结、不发布。
