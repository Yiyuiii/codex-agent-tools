...
10. Workflow单次执行：每个CI/release job只显式build一次，随后直接运行Vitest与已构建的release smoke；release smoke唯一一次`npm pack --dry-run --json`结果在内存中由包面、秘密与链接检查共同复用，不写入或上传未消费manifest，也不再第二次pack。CI不上传dist/runtime。
...
13. 明确保留：Job原子归属、唯一owner、句柄/环境/凭据隔离、parent/helper crash、managed与kernel分层测试、source→artifact/SHA/PE/resolver、canonical runtime-input digest、资格与能力双verifier、pack秘密/链接检查、beta/stable marker、公共beta精确安装、完整App重启/真实Stop、OIDC发布、`prepublishOnly`、固定Codex CLI来源及provider `KeyedLimiter`均不因删冗余而弱化。现有POSIX分支暂不在本轮删除，但不构成当前宿主发布承诺；若未来退役，必须把生产分支与相应测试作为独立产品范围变更一起处理。
...
3. **observer行为已完成**：`228c6a3 feat: run current-host acceptance observer`复用锁定Roslyn/net48 toolchain、`build.config.json`与x64 flags，实现完整双pipe旧/新宿主编排、held-handle真实exit FILETIME、严格EOF、五帧事件、marker absence与唯一receipt写入；MCP端自然终态排空和真实socket close也在同一提交闭合。observer及后续启动脚本不进入公开npm包，也不新增公开CLI/doctor命令；
...
四个职责批次都不得修改Job helper、fd3、`.mcp.json`、公开工具schema、活动配置或默认timeout；不得使用PID退出后重开、WMI/`ps`/Toolhelp scanner、`taskkill`或把人工文件当成受支持PASS。runtime内部证据、descriptor transport、observer行为与marker消费者合同均已完成；freeze前只剩checked-in artifact/build manifest、固定启动脚本和一次集成复审。同一Windows用户恶意篡改仓库、二进制或receipt不在本地ACL威胁模型内，但支持的固定流程必须由observer唯一写PASS并由stable verifier发现任何身份漂移。Node对pending named-pipe connect的取消能力与非重定向reparse tag识别只按当前固定私有路径合同处理，不为其它Node/Windows/机器增加兼容或安全承诺。
...
**原生observer第四子批状态（2026-08-02，行为已完成）：** `228c6a3 feat: run current-host acceptance observer`将`Program`接入raw stdin、固定脱敏失败输出与完整`HostAcceptanceObserverSession.Run`。首个受保护pipe只在DACL确认后发布`READY`，旧kernel peer与两层祖先handle持有后发布`OLD_HOST_BOUND`；三层真实退出、严格EOF、首pipe完整dispose并成功创建同名受保护第二pipe后才发布`OLD_HOST_EXITED`；新宿主首个请求帧到达后发布`REQUEST_STARTED`，五帧与严格EOF完成、所有pipe/connection/handle释放且marker确认不存在后，receipt才作为最后一步原子写入。MCP客户端自然终态会排空`HELLO + 5`，全局shutdown不能降级该终态，socket close只等真实`close`。fresh综合审阅无P1/P2；两个P3测试项已用held handle FILETIME精确比对和测试事件句柄释放闭合。主代理复验managed 34/34、kernel 7/7、客户端26/26、全库67文件1204 passed / 3平台条件skipped、类型、library build与diff PASS。`e922276`另将stdio cleanup测试绑定到同一当前宿主helper解析及真实`session_shutdown + ownedProcessDrained:true`证据。
...
**Beta.3 clean-tag 启动层复核与 beta.4 发布（2026-08-03，已完成）：** 从不可变`v0.1.1-beta.3`精确clean checkout启动时，`start-session.mjs`在observer前要求仓库存在被ignore的生成runtime，因此按预期fail closed，但也证明beta.3不能产生同一发布身份下的正式receipt。TDD修复把仓库对照集合缩为4个受版本控制制品；实际官方cache仍固定读取并摘要全部5个安装制品，生成runtime只从cache取，4个共同文件逐字节相等。旧tag不移动、不手工补产物；`0.1.1-beta.4`完整离线门禁通过69 files / 1235 passed / 5 skipped、类型、构建、release smoke与精确包闭包，随后完成PR/双重CI、OIDC发布、公共精确包验收与官方活动升级。当前installed/enabled beta.4缓存五文件摘要与marker一致；该启动层不属于canonical runtime、observer build input或能力指纹，故8 current / 0 legacy保持有效且没有重跑模型。下一步只做完整App重启与beta.4 observer真实Stop。
...
**资格执行状态（2026-08-03，已完成）：** 新批次 `2026-08-03T02-04-45.497Z-44fcbde6-a2bd-4f58-80c5-d723a374a923` 在 frozen commit `9054cbc45aaf1c91c2c62817244be5288032ede8` 上从唯一标准入口启动并完成八项；全部 passed、每项单次 client invocation、零 retry/fallback、owned process 全部排空，`promotionEligible=true`。immutable-evidence 与 frozen-candidate verifier 通过，manifest SHA256为`835224ccc7893d1e5f930bf2f63f29ef0bbb0a1daddaf7e85e807e626c79ecac`，不可变证据提交为`c09ce74`；现行索引已更新为8 current / 0 legacy。2026-08-02额度失败批次保持不可变历史，未补跑或改写。随后唯一完整`gate:offline`通过：69个测试文件、1224 passed / 5个平台条件skipped、release smoke与精确23-file pack通过。下一步是OIDC beta。
