...
**Beta.3 clean-tag 启动层复核（2026-08-03，beta.4 候选离线门禁已通过）：** 从不可变`v0.1.1-beta.3`精确clean checkout启动时，`start-session.mjs`在observer前要求仓库存在被ignore的生成runtime，因此按预期fail closed，但也证明beta.3不能产生同一发布身份下的正式receipt。TDD修复把仓库对照集合缩为4个受版本控制制品；实际官方cache仍固定读取并摘要全部5个安装制品，生成runtime只从cache取，4个共同文件逐字节相等。旧tag不移动、不手工补产物；新候选为`0.1.1-beta.4`。该启动层不属于canonical runtime、observer build input或能力指纹，故8 current / 0 legacy保持有效，不重跑模型。当前宿主完整`gate:offline`通过69 files / 1235 passed / 5 skipped、类型、构建、release smoke与精确包闭包。
