...
当前 `0.1.2-beta.2` 候选只调整调用方选型与插件界面：日常任务优先 Kimi/Ark Plan，API 计费的 Direct DeepSeek 保持显式选择；同时纠正公共 beta.1 宿主验收中把调用后全机非零进程误归因给当前调用的问题。模型/provider/credential/route、retry/fallback、公开工具 schema 与执行实现均未改变。canonical runtime digest 仍为 `06fbdd80...`，十项能力仍为 current。干净 `npm ci` 后的 `gate:offline` 为 72 files passed、1289 passed / 5 skipped / 0 failed，release smoke 通过；隔离官方插件 add/list/cache MCP/remove 生命周期通过，报告明确绑定 beta.2 与实际执行日期。Ark Coding Plan 单次只读外审返回 completed、实际模型 `ark-code-latest`、无文件变化，结论无 P0/P1；两项 P2 已修复。新 beta marker 绑定干净依赖闭包生成的版本化插件树，发布和活动升级尚未执行。
