...
当前 `0.1.2-beta.1` 已新增 Direct DeepSeek。本清单以下“8 current”均描述 `0.1.1` 发布快照。2026-08-23 修复后的七项定向 batch `2026-08-23T11-11-13.828Z-f5c1cb1c-9b94-4dee-8c2f-b3f5d6098e60` 在 frozen commit `388f0fdc37db02b5c6104988aa68baa793eda791` 上 7/7 passed；每项单次调用、零 retry/fallback、owned process drained，manifest SHA-256 为 `968c00d5ecf9a612fd8e9b5bff34ef84c31126ac6598078d9f9c74ee4f48e550`。immutable-evidence 与 detached frozen-candidate verifier 均通过，索引更新后机器分析为 10 current / 0 stale / 0 legacy，`verify:capabilities` 与 release smoke 通过。此前广覆盖误跑与首次定向预模型阻断保持不可变。PR #19 与 `next` push CI 全绿，release run `32638706081` 通过 GitHub Actions OIDC 发布并验证 `next=0.1.2-beta.1`，GitHub prerelease 已创建；公共精确包隔离验收全绿，见 [0.1.2-beta.1 npm 验收](0.1.2-beta.1-npm-acceptance.md)。活动 App 仍只加载本地 staging，未因发布自动升级。

`0.1.2-beta.2` 只调整调用方选型与插件界面：日常任务优先 Kimi/Ark Plan，API 计费的 Direct DeepSeek 保持显式选择；同时纠正公共 beta.1 宿主验收中把调用后全机非零进程误归因给当前调用的问题。模型/provider/credential/route、retry/fallback、公开工具 schema 与执行实现均未改变。canonical runtime digest 仍为 `06fbdd80...`，十项能力仍为 current。干净 `npm ci` 后的 `gate:offline` 为 72 files passed、1289 passed / 5 skipped / 0 failed，release smoke 通过；隔离官方插件 add/list/cache MCP/remove 生命周期通过。Ark Coding Plan 单次只读外审返回 completed、实际模型 `ark-code-latest`、无文件变化，结论无 P0/P1；两项 P2 已修复。PR #21 与 `next` push CI 全绿，Release run `32643186815` 通过 GitHub Actions OIDC 发布并验证 `next=0.1.2-beta.2`，GitHub prerelease 指向 `dac733b863548602758b071293ad3966102cf6a5`。公共精确包隔离验收全绿且真实模型调用为 0，见 [0.1.2-beta.2 npm 验收](0.1.2-beta.2-npm-acceptance.md)。活动升级尚未执行。
...
晋级顺序已经闭合：beta.4 公共身份与仓库内风险决策被 stable marker 精确绑定；Node 24 离线门禁、main PR/CI、稳定标签 OIDC 发布、公共 stable 隔离复验和活动官方插件升级均完成。禁止本地 `npm publish`；未来候选版本仍须先查询 registry，不得猜测或复用已发布版本。`v0.1.1-beta.2` 与 `v0.1.1-beta.3` 的历史阻断保持不可变。

## `0.1.1` 发布快照状态总览

| 层级 | 验收对象                                  | 当前状态                                                                    | 通过证据路径                                                                                                                                                                                                               |
| ---- | ----------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | 确定性单测、类型检查、构建、release smoke | passed：69 files / 1246 passed / 5 platform skips，release smoke passed    | 2026-08-07 stable `gate:offline`；当前 Node 24 宿主与精确 23-file pack                                                                                                                                                       |
...
| 4    | 活动 Codex 的真实 App 宿主门禁            | skipped / unverified：维护者终止普通 Stop 复验；没有 PASS receipt             | 公共包证据为 [0.1.1-npm-acceptance.md](0.1.1-npm-acceptance.md)；权限包为 [real-plugin-install-review.md](real-plugin-install-review.md)；宿主结果为 [real-host-acceptance.md](real-host-acceptance.md) |
...
该入口复用第1层已经构建的候选。脚本必须在唯一临时 `CODEX_HOME` 中完成官方 marketplace add/list、plugin add/list、从官方缓存副本启动 MCP、plugin remove/list 与 marketplace remove/list。调用门禁先证明已退役的 `gemini-3.5-flash` 以 unknown logical LLM 被明确拒绝、错误精确列出五项登记 LLM 且没有启动 Pi，再分别用 qualified 的 Direct `deepseek-v4-flash` 与 `ark-agent-deepseek-v4-flash` 验证从官方缓存副本各恰好调用 fake Pi 一次、direct 路由、父代理清除、逐路线目标凭据隔离和进程清理。脚本还必须验证 MCP 契约和官方列表语义回滚。单独运行第2层时改用`npm run acceptance:plugin:isolated`先构建一次。
...
- 只执行 checked-in observer 输出的两次精确宿主调用：旧宿主的 `kimi-k3`
  `external_review` 握手，以及新宿主在 nonce 隔离仓库中的 `kimi-k3`
  `external_delegate`；不额外重跑能力资格或泛化 Kimi/Pi 代表性测试；
- observer 发布 `REQUEST_STARTED` 后才使用 App 普通 Stop；
- 唯一 PASS receipt 精确证明请求为 cancelled、completion marker absent、handler 与
  in-flight 闭合、owned descendants zero，且所有结果和诊断脱敏。
...
- 本轮不调用或修改 Claude Code；beta 与 stable 都只通过 GitHub Actions OIDC 发布，
  没有本地手工 `npm publish`，也不发布公共 marketplace。
