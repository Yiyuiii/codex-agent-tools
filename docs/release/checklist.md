...
当前结论：**当前宿主离线实现、原生辅助层、observer、freeze、四模型八项真实资格与离线 release gate 均已完成；现行能力索引为 8 current / 0 legacy。`0.1.1-beta.3` 已由 GitHub Actions OIDC 发布到 npm `next`，通过公共精确包隔离验收并升级为活动 installed/enabled 插件，但其精确 clean-tag 启动层在 observer 前错误要求 Git 未跟踪的生成 runtime，因而 fail closed。`0.1.1-beta.4` 候选修复为“4 个标签受控文件逐字节对照 + 5 个实际缓存制品完整摘要”；它完成 OIDC 发布、公共包验收、官方升级、完整 App 重启，并在 observer 的 `REQUEST_STARTED` 后由普通 Stop 取得 `cancelled + owned-zero` receipt 前，stable 继续阻断。**
...
晋级顺序只有一条：`0.1.1-beta.3` 的 OIDC 发布、公共 npm 精确版本验收与官方插件升级已经完成，但 clean-tag 启动层缺口要求新发 `0.1.1-beta.4`。beta.4 已通过当前宿主离线门禁；接下来必须完成 PR/双重 CI、精确标签与 OIDC 发布，再完成公共 npm 精确版本验收和官方插件升级；随后完整退出并重开 App，在 observer 发布 `REQUEST_STARTED` 后使用普通 Stop，并证明 receipt 为 `cancelled + owned-zero`；最后才由 GitHub Actions OIDC 发布 stable。禁止本地 `npm publish`；未来候选版本仍须先查询 registry，不得猜测或复用已发布版本。`v0.1.1-beta.2` 标签在 npm publish 前因 plugin tree 原始字节与 GitHub checkout 不一致而 fail closed，版本未进入 npm，标签不移动、不复用。
...
| 4    | 活动 Codex 的真实 App 宿主门禁            | partial：beta.3 启动层在 observer 前 fail closed；待 beta.4 发布/安装、完整重启与普通 Stop receipt | 历史公共包证据为 [0.1.1-beta.3-npm-acceptance.md](0.1.1-beta.3-npm-acceptance.md)；权限包为 [real-plugin-install-review.md](real-plugin-install-review.md)；宿主结果为 [real-host-acceptance.md](real-host-acceptance.md) |
