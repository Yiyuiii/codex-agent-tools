...
当前结论：**当前宿主离线实现、原生辅助层、observer、固定启动层、freeze、四模型八项真实资格与离线 release gate 均已完成；现行能力索引为 8 current / 0 legacy。`0.1.1-beta.3` 已由 GitHub Actions OIDC 发布到 npm `next`，通过公共精确包隔离验收，并已按官方 remove/add 升级为活动 installed/enabled 插件；旧 `codex_cc_tools` 仍 enabled。完整 App 重启，并在 observer 发布 `REQUEST_STARTED` 后由普通 Stop 取得 `cancelled + owned-zero` receipt 前，stable 继续阻断。**
...
| 4    | 活动 Codex 的真实 App 宿主门禁            | partial：活动 beta.3 已安装；待完整重启与普通 Stop 的 cancelled/owned-zero receipt | 公共包证据为 [0.1.1-beta.3-npm-acceptance.md](0.1.1-beta.3-npm-acceptance.md)；权限包为 [real-plugin-install-review.md](real-plugin-install-review.md)；宿主结果为 [real-host-acceptance.md](real-host-acceptance.md) |
