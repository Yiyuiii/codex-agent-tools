...
诊断会检查：Kimi 可执行文件、版本与登录状态；本包拥有的 MCP 注册；公开工具名；各逻辑 LLM 的真实模型、运行时、固定网络路由和 review/delegate 质量门禁。
...
这是必要步骤，因为 MCP 配置固定保存当前 Node 和包入口的绝对路径。安装器不会要求用户维护 Pi 配置；未来 Pi 运行时使用的模型目录和生成配置由本包版本管理。
