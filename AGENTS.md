...
- 2026-07-26：任务 8 的 [真实插件安装审阅状态包](docs/release/real-plugin-install-review.md) 已形成 `blocked / not ready` 草案，记录原始 8 passed / 2 failed、成对注册表 6 passed / 4 pending、两个失败 evidence、十门禁重入条件、隔离状态差异及未来官方回滚边界。qualified 的 Kimi K3 与 Ark Agent Plan 已依次完成只读外审，二者都没有改动文件；审阅未发现阻断项，仅发现发布清单状态措辞和运维文档硬编码日期两项 Minor，现已修正。独立规格复审已经通过；独立质量复审随后发现真实 App 工具发现范围可能被误解为全局唯一、清单历史日期被抹去，以及临时缓存路径可能被误作未来版本承诺。状态包现已把工具范围限定到新安装的 `codex_external_agents` 插件归属，保留清单的建立与最近复核日期，并要求未来 ready 重入时用当时 CLI 和候选包重新隔离取证，不能仅凭本轮旧路径判定合法宿主差异失败。任务 8 尚待质量复审复核与最终验证；当前不提出或接受真实安装许可，不执行任务 9。
