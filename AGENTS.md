...
- 2026-07-26：任务 8 的 [真实插件安装审阅状态包](docs/release/real-plugin-install-review.md) 已形成 `blocked / not ready` 草案，记录原始 8 passed / 2 failed、成对注册表 6 passed / 4 pending、两个失败 evidence、十门禁重入条件、隔离状态差异及未来官方回滚边界。qualified 的 Kimi K3 与 Ark Agent Plan 已依次完成只读外审，二者都没有改动文件；Kimi 只指出发布清单状态措辞，Ark Agent Plan 只指出 `docs/operations.md` 顶部硬编码的 as-of 日期，后者已通过删除该硬编码日期修复。独立规格复审已经通过；独立质量复审随后发现真实 App 工具发现范围可能被误解为全局唯一、发布清单没有区分建立日期与最近复核日期，以及临时缓存路径可能被误作未来版本承诺。第一轮修复已限定新插件工具归属、分别标注清单的建立与最近复核日期，并要求未来 ready 重入重新隔离取证；质量复核又发现本轮隔离 add 的配置主体曾被误写为活动 `config.toml`，现已更正为临时 `CODEX_HOME` 内的 `config.toml`，并厘清外审与质量复审的日期问题归因。任务 8 尚待质量复审复核与最终验证；当前不提出或接受真实安装许可，不执行任务 9。
